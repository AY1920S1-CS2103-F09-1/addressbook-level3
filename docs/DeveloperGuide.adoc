...
The logic of the algorithm is implemented in the `graph` package which resides in the `logic` package. The algorithm is run by the `ScheduleCommand` which serves as the driver class of the algorithm.
The command is then invoked when the user key in the relevant keyword (please refer the user guide).

Details of the implementation of the algorithm and the diagram of the `graph` component will be up soon.
