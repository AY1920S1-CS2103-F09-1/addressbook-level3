...
. Expected: Schedules in the `Schedules` tab reflects the scheduled result accordingly, i.e. the name of the
  interviewee shows up in the interview schedule if its availability falls in one of the available interview slots
  ...
  "Interviewees that are not allocated a slot" followed by the name of the interviewees that are not allocated slot,
  or "No matching is found :(".

. You can use the sample test data below to test if the scheduling feature is working properly.
... All interviewees are allocated a slot (<<Interviewers data>>, <<Interviewees data>>).
... Some interviewees are allocated a slot (<<Interviewers data>>, <<Interviewees data>>).
... All interviewees are not allocated a slot (<<Interviewers data>>, <<Interviewees data>>).

. You can also design your own test data to check if the scheduling feature is working properly by adding some available
  interview slots in the interviewer's availability template. Then, add interviewees which have availabilities that fall in the
...
.. You can also test the opposite way, i.e. design test data which contains interviewees that cannot be allocated a slot and check
   if the name of the interviewee exists in the schedule.
